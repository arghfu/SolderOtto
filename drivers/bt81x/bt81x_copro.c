...
void bt81x_copro_cmd_button(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t font, uint16_t options, const char* s)
{
    const uint16_t str_bytes = strlen(s) + 1;
    const uint16_t padding_bytes = (4 - (str_bytes % 4)) % 4;
    const uint16_t cmd_size = sizeof(CMD_BUTTON) +
        sizeof(x) +
        sizeof(y) +
        sizeof(w) +
        sizeof(h) +
    ...
    bt81x_wr32(BT81X_RAM_CMD + reg_cmd_write, CMD_BUTTON);
    increase_reg_cmd_write(sizeof(CMD_BUTTON));
    ...
    bt81x_wr16(BT81X_RAM_CMD + reg_cmd_write, w);
    increase_reg_cmd_write(sizeof(w));

    bt81x_wr16(BT81X_RAM_CMD + reg_cmd_write, h);
