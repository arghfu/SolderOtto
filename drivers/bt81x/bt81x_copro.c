...
{
	reg_cmd_read = bt81x_rd32(FT800_REG_CMD_READ);
}

static void flush_reg_cmd_write(void)
{
	bt81x_wr32(FT800_REG_CMD_WRITE, reg_cmd_write);
}
...
void bt81x_copro_cmd(uint32_t cmd)
{
	...
	bt81x_wr32(FT800_RAM_CMD + reg_cmd_write, cmd);
			   ...
			   const char *s)
{
	...
	bt81x_wr32(FT800_RAM_CMD + reg_cmd_write, CMD_TEXT);
	increase_reg_cmd_write(sizeof(CMD_TEXT));

	bt81x_wr16(FT800_RAM_CMD + reg_cmd_write, x);
	increase_reg_cmd_write(sizeof(x));

	bt81x_wr16(FT800_RAM_CMD + reg_cmd_write, y);
	increase_reg_cmd_write(sizeof(y));

	bt81x_wr16(FT800_RAM_CMD + reg_cmd_write, font);
	increase_reg_cmd_write(sizeof(font));

	bt81x_wr16(FT800_RAM_CMD + reg_cmd_write, options);
	increase_reg_cmd_write(sizeof(options));

	(void)bt81x_drv_write(FT800_RAM_CMD + reg_cmd_write, s, str_bytes);
			     ...
			     int32_t n)
{
	...
	bt81x_wr32(FT800_RAM_CMD + reg_cmd_write, CMD_NUMBER);
	...
	bt81x_wr32(FT800_RAM_CMD + reg_cmd_write, n);
...
void bt81x_copro_cmd_calibrate(uint32_t *result)
{
	...
	bt81x_wr32(FT800_RAM_CMD + reg_cmd_write, CMD_CALIBRATE);
